...
parser = argparse.ArgumentParser(
    description="Plot periodic trends as a heat "
    "map over the periodic table of elements"
)
parser.add_argument(
    "filename", type=str, help="Filename (with extension) of " "CSV-formatted data"
)
parser.add_argument(
    "--width", type=int, default=1050, help="Width (in pixels) of " "figure"
)
parser.add_argument(
    "--cmap_choice",
    type=int,
...
parser.add_argument(
    ...
    type=float,
    default=0.65,
    help="Alpha value " "for color scale (ranges from 0 to 1)",
)
parser.add_argument(
    "--extended",
    default="True",
...
parser.add_argument(
    "--cbar_height", type=int, help="Height (in pixels) of color " "bar"
)
parser.add_argument(
    "--cbar_standoff",
    type=int,
    help="Distance (in pixels) that the "
    "colorbar tick values should be from the colorbar itself",
)
parser.add_argument(
    "--cbar_fontsize",
    type=int,
    help="Fontsize (in pt) that the " "colorbar tick values should be",
)
...
if log_scale == 0:
    ...
    for datum in data:
        if datum < 0:
            raise ValueError(
                "Entry for element " + datum + " is negative but"
